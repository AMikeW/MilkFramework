...
namespace UnityGameFramework.Runtime
{
    ...
    public sealed class UIForm : MonoBehaviour, IUIForm
    {
        ...
        public void OnInit(int serialId, string uiFormAssetName, IUIGroup uiGroup, bool pauseCoveredUIForm, bool isNewInstance, string luaRelativePath, object userData)
        {
            m_SerialId = serialId;
            m_UIFormAssetName = uiFormAssetName;
            m_UIGroup = uiGroup;
            ...
            try
            {
                m_UIFormLogic.OnInit(luaRelativePath, userData);
            }
            catch (Exception exception)
            {
                Log.Error("UI form '[{0}]{1}' OnInit with exception '{2}'.", m_SerialId.ToString(), m_UIFormAssetName, exception.ToString());
